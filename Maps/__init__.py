...
def create_nav_graphs(nav):
    ...
    for map_name in nav:
        ...
        with open("mapMetaData/de_inferno.txt", encoding="utf8") as edge_list:
        ...
        for line in edge_list_lines:
            ...
            map_graph.add_edge(
                ...
                int(areas[1]),
                weight=np.sqrt(
                    np.array(map_graph.nodes[int(areas[0])]["center"]) ** 2
                    + np.array(map_graph.nodes[int(areas[1])]["center"]) ** 2
                ),
            )
